...
[English](https://github.com/Unihra/unihra_sdk/blob/main/README.md) · Русский
...
- **Неудачные страницы** — список URL конкурентов, которые вернули пустой, заблокированный или ненадежный результат парсинга.
...
print(f"Страниц не вошло в анализ: {len(failed)}")
for p in pages:
...
Отчет содержит листы: *Page Structure*, *Umbrella Gaps*, *Word Analysis*, *N‑Grams*, *Anchors*, *Parsed Pages*, *Failed Pages* и — при `triplet_analysis=True` — *Triplets* и *Triplets Gaps*.
...
**Как это работает:** Инструмент `unihra_analyze` запускает полный анализ и сохраняет результат локально, возвращая только `result_id` и краткую сводку. Затем вы используете инструменты `unihra_get_*` вместе с `result_id` для извлечения конкретных разделов по запросу: покрытие URL, зонтичные пробелы, анкоры, слова, n‑граммы, триплеты (Граф знаний) или структура страницы. Это позволяет изучить полный отчёт раздел за разделом.
...
| `unihra_get_page_status` | Показывает запрошенные URL, которые вошли и не вошли в анализ |
