...
**Файловая архитектура:** Инструмент `unihra_analyze` запускает полный анализ и сохраняет объемный сырой результат (~200 тыс. символов) в локальный JSON-файл, возвращая LLM-модели только `result_id` и краткую сводку. Затем модель использует инструменты `unihra_get_*` вместе с `result_id` для извлечения конкретных срезов данных по запросу. Это позволяет сохранить компактный контекст модели, не теряя при этом никаких данных.
